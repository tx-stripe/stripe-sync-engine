...
echo "   Creating 600 products to ensure sync takes sufficient time..."
echo ""
...
echo "   ✓ All 600 products created"
echo ""
echo "✓ Test data created in Stripe (600 products)"
echo ""
...
echo "   ✓ Products synced before crash: $PRODUCTS_SYNCED / 600"
...
if [ "$FINAL_PRODUCTS" -ge 600 ]; then
    echo "   ✓ All products synced: $FINAL_PRODUCTS / 600"
else
    echo "   ❌ Expected 600 products, found $FINAL_PRODUCTS"
    exit 1
fi
...
if [ "$ACTUAL_TEST_PRODUCTS" -eq 600 ]; then
    echo "   ✓ No data lost - all 600 test products in database"
else
    echo "   ❌ Expected 600 test products, found $ACTUAL_TEST_PRODUCTS"
    exit 1
fi
...
echo "- ✓ All 600 products synced with no data loss"
