...
/**
 * Get current account command - retrieves the current Stripe account (cached or from API).
 */
export async function getAccountCommand(
  ...
  try {
    ...
    if (format === 'table') {
      console.log(chalk.blue('Fetching current Stripe account...'))
      console.log(chalk.gray(`Schema: ${schema}`))
      console.log(chalk.gray(`Database: ${databaseUrl.replace(/:[^:@]+@/, ':****@')}`))
    }
    ...
    if (!account) {
      console.log(chalk.yellow('No account found'))
      return
    }
    ...
    if (format === 'json') {
      ...
      console.log(chalk.cyan('Email:        ') + (account.email || 'N/A'))
      console.log(
        chalk.cyan('Business:     ') + (account.business_profile?.name || 'N/A')
      )
      console.log(chalk.cyan('Country:      ') + (account.country || 'N/A'))
      console.log(
        chalk.cyan('Type:         ') + (account.type || 'N/A')
      )
      console.log(chalk.gray('─'.repeat(60)))
    }
  } catch (error) {
...
/**
 * List accounts command - retrieves all synced Stripe accounts from the database.
 */
export async function listAccountsCommand(
  options: CliOptions,
  format: 'json' | 'table' = 'table'
): Promise<void> {
  try {
    ...
    if (format === 'table') {
      console.log(chalk.blue('Listing synced Stripe accounts...'))
      console.log(chalk.gray(`Schema: ${schema}`))
      console.log(chalk.gray(`Database: ${databaseUrl.replace(/:[^:@]+@/, ':****@')}`))
    }
    ...
    // Create StripeSync instance (API key not needed for listing)
    const stripeSync = new StripeSync({
      databaseUrl,
      schema,
      stripeSecretKey: 'sk_test_placeholder', // Not used for listing
    ...
    if (format === 'json') {
      ...
      if (accounts.length === 0) {
        console.log(chalk.yellow('\nNo synced accounts found'))
        return
      }

      console.log(chalk.blue(`\nSynced Stripe Accounts (${accounts.length}):`))
      ...
      accounts.forEach((account, index) => {
        ...
        console.log(chalk.cyan('ID:       ') + account.id)
        console.log(chalk.gray('Email:    ') + (account.email || 'N/A'))
        console.log(
          chalk.gray('Business: ') + (account.business_profile?.name || 'N/A')
        )
        console.log(chalk.gray('Country:  ') + (account.country || 'N/A'))
        console.log(chalk.gray('Type:     ') + (account.type || 'N/A'))
      })
