...
export class PostgresClient {
  ...
  /**
   * Check if stripe-sync is installed.
   *
  ...
  async isInstalled(): Promise<boolean> {
    try {
      ...
      const schemaExists = schemaCheck.rows[0]?.schema_exists === true
      ...
      const migrationsCheck = await this.query(
        `SELECT EXISTS (
          SELECT 1 FROM information_schema.tables
          WHERE table_schema = $1 AND table_name IN ('migrations', '_migrations')
        ) as table_exists`,
        [this.config.schema]
      )

      const migrationsTableExists = migrationsCheck.rows[0]?.table_exists === true
      ...
      if (!comment || !comment.includes('stripe-sync')) {
        throw new Error(
          `Legacy installation detected: Schema '${this.config.schema}' and migrations table exist, but missing stripe-sync comment marker. ` +
